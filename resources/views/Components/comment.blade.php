<div class="flex gap-3 p-3 rounded-xl {{ $comment->parent_id ? 'ml-8' : '' }}" data-comment-id="{{ $comment->id }}">
    ...
    <div class="flex-1">
        ...
        <div class="mt-2 flex items-center gap-4">
            ...
            <button class="reply-button flex items-center gap-1.5 text-sm text-gray-400 hover:text-blue-500 transition-colors"
                    data-comment-id="{{ $comment->id }}">
                <i class="ri-chat-1-line text-lg"></i>
                <span>Reply</span>
            </button>
            @if($comment->user_id === auth()->id())
                <button class="delete-comment flex items-center gap-1.5 text-sm text-gray-400 hover:text-red-500 transition-colors">
